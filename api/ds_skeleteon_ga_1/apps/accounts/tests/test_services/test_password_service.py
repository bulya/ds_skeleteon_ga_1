...
from ds_skeleteon_ga_1.apps.accounts.exceptions import (
    InvalidPasswordError,
    InvalidResetPasswordSignatureError,
    WrongPasswordError,
)
from ds_skeleteon_ga_1.apps.accounts.models import UserAccount
from ds_skeleteon_ga_1.apps.accounts.services.password import PasswordService


PASSWORD_SERVICE_PATH = "ds_skeleteon_ga_1.apps.accounts.services.password.PasswordService"  # nosec
...
def test_reset_password_signature_failure(mocker, settings, exception):
    ...
    settings.ds_skeleteon_ga_1_RESET_PASSWORD_EXPIRATION_DELTA = 42
    mocked_change_password = mocker.patch(f"{PASSWORD_SERVICE_PATH}.change_password")
